...
class UnetFormerLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            ...
            if isinstance(logit_main,list) and isinstance(logit_aux,list):
                main_loss = 0
                aux_loss = 0
                for i in range(len(logit_main)):
                    main_loss += self.main_loss(logit_main[i], labels[i])
                    aux_loss += self.aux_loss(logit_aux[i], labels[i])
